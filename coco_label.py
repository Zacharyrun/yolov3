...
dataDir = 'H:/Other_Datasets/coco/'  # COCO数据集所在的路径
dataType = 'train2017'  # 要转换的COCO数据集的子集名
annFile = '%s/annotations/instances_%s.json' % (dataDir, dataType)  # COCO数据集的标注文件路径
classes = get_classes_and_index('H:/Other_Datasets/coco/coco.csv')

# labels 目录若不存在，创建labels目录。若存在，则清空目录
if not os.path.exists('%s/labels/%s/' % (dataDir, dataType)):
    os.makedirs('%s/labels/%s/' % (dataDir, dataType))
else:
    shutil.rmtree('%s/labels/%s/' % (dataDir, dataType))
    os.makedirs('%s/labels/%s/' % (dataDir, dataType))
...
for imgId in imgIds:
    ...
    for annId in annIds:
        ...
        if cat in classes:
            objCount = objCount + 1
            out_file = open('%s/labels/%s/%s.txt' % (dataDir, dataType, filename[:-4]), 'a')
